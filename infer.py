...
def main(args):
    gpu_u = args.gpu >= 0 and torch.cuda.is_available()
    if gpu_u:
    ...
    test_data = data.test
    coords = data.coords
    rel_dict = data.rel_dict
    num_rels = data.num_rels
    grid_dict = data.grid

    boundaries = utils.dynamic_dist(args.bin_num, coords, train_data)

    log = str(args) + '\n'
    # create model
    model = EGNNPred(num_nodes, args.n_hidden, num_rels,
    ...
    hop2_dict = utils.build_hop2_dict(train_data, rel_dict, args.dataset, path_len=2)
    test_graph = utils.generate_sampled_hetero_graphs_and_labels(train_data, num_nodes, rel_dict, hop2_dict,
                                                                 coords=coords, test=True)
    ...
    if gpu_u:
        ...
        valid_data_list = utils.select_time_data(valid_data_list, rel_dict)
        test_data_list = utils.select_time_data(test_data_list, rel_dict)
        train_data_list = utils.select_time_data(torch.from_numpy(train_data).cuda(), rel_dict)
    ...
    while True:
        model.train()
        epoch += 1
        g, data, labels = utils.generate_sampled_hetero_graphs_and_labels(train_data, num_nodes, rel_dict,
                                                                          hop2_dict=hop2_dict,
                                                                          coords=coords,
        ...
        feature_inputed = model(g, node_id)
        data_labels = torch.cat([data, labels], dim=1)
        data_labels = utils.select_time_data(data_labels, rel_dict)
        data, labels = [data_labels[i][:, :3] for i in range(4)], [data_labels[i][:, 3].float() for i in range(4)]
        loss = model.get_loss(g, feature_inputed, data, labels)
        loss.backward()
        ...
        if epoch % args.evaluate_every == 0:
            ...
            valid_metrics_list = []
            with torch.no_grad():
                feature_inputed = model(test_graph, node_id)
                for ii, (trn_data, tst_data, val_data) in enumerate(
                        zip(train_data_list, test_data_list, valid_data_list)):
                    valid_ranks = model.select_rs(feature_inputed[ii], val_data, trn_data, tst_data)
    ...
    with torch.no_grad():
        ...
        for ii, (trn_data, tst_data, val_data) in enumerate(zip(train_data_list, test_data_list, valid_data_list)):
            test_ranks = model.select_rs(feature_inputed[ii], tst_data, trn_data, val_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--trained-model", type=str, default='ssl_model')
    parser.add_argument("--size-grid", type=int, default=300)
