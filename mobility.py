...
def load_mobility_dict_with_time(trips, stations):
    ...
    for index, row in trips.iterrows():
        ...
        if start_zone != end_zone:
            continue
        cnt += 1
        delta = (datetime.datetime.strptime(end_time, "%m/%d/%Y %H:%M:%S") - datetime.datetime.strptime(start_time,
                                                                                                        "%m/%d/%Y %H:%M:%S")).seconds / 60
        ids = (start_id, end_id)
...
def build_interaction(flow_sorted_list, flow_dict, stat_dict):
    ...
    for k1, v1 in flow_dict.items():
        for k2, v2 in v1.items():
            ...
            for i in range(4):
                ...
                if len(v2[i]) > threshold and len(flow_dict[k2][k1][i]) > threshold and 1.0 * len(v2[i]) / (
                cnt_dict[k1]) > 0.0005 and 1.0 * k1v / (cnt_dict[k2]) > 0.0005:
                    rel[i] += [(str(k1), str(k2), len(v2[i]))]
                    if k1 < k2:
                        rel_set[i].add((k1, k2))
                    else:
                        rel_set[i].add((k2, k1))
    ...
    return node_dict, rel, all_pair_list_d


def build_dataset(node_dict, rel, all_pair_list_d):
    ...
    for i in range(4):
        for u, v, n in rel[i]:
            ...
            value_dict[v + '#' + u] = n
            if v not in rel_g:
                rel_g[v] = {'time1': {}, 'time2': {}, 'time3': {}, 'time4': {}}
            rel_g[v]['time' + str(i + 1)][u] = n
    rel_g_skip = dict()
    for k1, v in rel_g.items():
        rel_g_skip[k1] = {}
        for t, v2 in v.items():
            if len(v2) == 0:
                rel_g_skip[k1][t] = ''
    ...
    for i in range(4):
        ...
        while len(test_val_pair_dict['time' + i]) < num_test:
            ...
            for j in range(int(i), 5):
                if len(rel_g[b1]['time' + str(j)]) <= 1 or len(rel_g[b2]['time' + str(j)]) <= 1:
                    conflict = True
                    break
            if conflict: continue
            for j in range(int(i), 5):
                if b2 not in rel_g[b1]['time' + str(j)]:
                    continue
                test_val_pair_dict['time' + str(j)].add(lll[ind])
                rel_g[b1]['time' + str(j)].pop(b2)
                rel_g[b2]['time' + str(j)].pop(b1)
...
def save_interaction_dataset(output_path, node_dict, value_dict, mobi_dict, train_pair_dict, valid_pair_dict,
                              test_pair_dict):
    times = ['morning', 'midday', 'night', 'late-night']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--raw-in', type=str, default='./data/raw/DIVVY')
    parser.add_argument('--out', type=str, default='./data/chicago/')
    ...
    node_dict, rel, all_pair_list_d = build_interaction(mobi_sort_list, mobi_dict, stat_dict)
    train, valid, test, value_dict = build_dataset(node_dict, rel, all_pair_list_d)

    save_interaction_dataset(args.output_path, node_dict, value_dict, mobi_sort_list, train, valid, test)
